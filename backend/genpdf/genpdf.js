...
async function generateRabiesExposurePDF(data, res) {
  try {
    ...
    // Header
    drawText('ENREGISTREMENT', margin, y, { bold: true });
    drawText('CPC030-PR002-EN01', margin + 150, y);
    y -= lineHeight;
    drawText('Déclaration de Cas d’Exposition : RAGE', margin, y, { bold: true, size: 12 });
    y -= lineHeight;
    drawText('Version : 2', margin, y);
    drawText('Date d’application : Juin 2025', margin + 150, y);
    y -= 2 * lineHeight;
    ...
    drawText(data.animal_description_info?.labResult || '', margin + 125, y);
    ...
    // DOSSIER MEDICAL
    drawText('DOSSIER MEDICAL', margin, y, { bold: true });
    y -= lineHeight;
    ...
    drawCheckbox(margin + 160, y, data.medical_dossier_info?.coagulationIssue === 'Non');
    drawText('Non', margin + 175, y);
    drawCheckbox(margin + 210, y, data.medical_dossier_info?.coagulationIssue === 'Oui');
    drawText('Oui : précisez', margin + 225, y);
    drawLine(margin + 300, y, 250);
    ...
    drawText('Oui si oui Date', margin + 345, y);
    drawLine(margin + 420, y, 50);
    drawText(data.medical_dossier_info?.antirabiesDate || '', margin + 425, y);
    drawText('N°Lot', margin + 480, y);
    drawLine(margin + 510, y, 70);
    drawText(data.medical_dossier_info?.antirabiesLot || '', margin + 515, y);
    y -= lineHeight;
    ...
    // Footer
    drawText('Revue des données : Chef service Vaccination', margin, margin);
    drawText('1/1', 595 - margin - 20, margin);
