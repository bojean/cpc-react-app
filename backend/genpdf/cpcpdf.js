  ...
  try {
    ...
    return {
      ...
      medical_dossier_info: {
        ...
        signature: row.signature, // Base64-encoded PNG string
...
async function generateRabiesExposurePDF(data, res) {
  try {
    ...
    y -= lineHeight * 2;
    // DOSSIER MÉDICAL section boundary
    sectionHeight = sectionStartY - y + lineHeight * 3;
    page.drawRectangle({ x: margin - 5, y: y - 5, width: 595 - 2 * margin + 10, height: sectionHeight + 10, borderWidth: 1, borderColor: rgb(0, 0, 0) });
    y -= lineHeight * 2;

    // Footer
    drawText('Revue des données : Chef service Vaccination', margin, margin - 10);
    drawText('1/1', 595 - margin - 20, margin - 10);

    // Add signature image to the right of "Chef service Vaccination"
    if (data.medical_dossier_info?.signature && data.medical_dossier_info.signature.startsWith('data:image/png;base64,')) {
      const base64Image = data.medical_dossier_info.signature.replace('data:image/png;base64,', '');
      const signatureImage = await pdfDoc.embedPng(base64Image);
      const signatureWidth = 100; // Adjust width as needed
      const signatureHeight = signatureImage.height * (signatureWidth / signatureImage.width);
      page.drawImage(signatureImage, {
        x: 595 - margin - signatureWidth - 20, // Right of "Chef service Vaccination"
        y: margin - signatureHeight, // Align with the bottom of the text
